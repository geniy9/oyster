...
function logoCorner(isNav) {
  ...
  gsap.to(logo3DContainer.value, {
    y: isMobile ? '0px' : isNav ? '-128px' : '0px',
    bottom: '4px',
    right: isMobile ? '4px' : '80px',
    xPercent: 0,
    yPercent: 0,
    width: '60px',
    height: '60px',
...
onMounted(async () => {
  await nextTick()
  const triggerEl = document.querySelector('#footerSection')
  if (!triggerEl || !logo3DContainer.value) return
...
<template>
  <div>
    ...
    <div ref="logo3DContainer" class="fixed z-60">
      <NuxtLinkLocale to='/#hero'>
        <LogoModel />
      </NuxtLinkLocale>
    </div>
