...
const props = defineProps({
  ...
  imageClass: { type: String, default: "w-72 h-128" } 
});
...
function handleSwipe() {
  const threshold = 50;
  if (touchEndX.value < touchStartX.value - threshold) changeSlide("next");
  if (touchEndX.value > touchStartX.value + threshold) changeSlide("prev");
}
...
function changeSlide(target) {
  ...
  tl.to([currentTitle, currentDesc], { yPercent: -100, duration: 0.6, ease: "power2.in", stagger: 0.05 }, 0);
  tl.to(currentImg, { scale: 1, opacity: 0, duration: 0.6, ease: "power2.in" }, 0);
  tl.to(nextImg, { scale: 1, opacity: 1, duration: 0.8, ease: "power2.out" }, 0.2);
  tl.to([nextTitle, nextDesc], { yPercent: 0, duration: 0.8, ease: "power2.out", stagger: 0.05 }, 0.3);
}
...
<template>
  <div class="relative w-full min-h-125 flex flex-col touch-pan-y" @touchstart="handleTouchStart" @touchend="handleTouchEnd">
    <div class="relative grid grid-cols-1 w-full grow">
      <div v-for="(item, index) in slides" :key="item.index" :ref="(el) => { if (el) slideRefs[index] = el; }" class="col-start-1 row-start-1 w-full h-full flex items-center" style="visibility: hidden">
        <div class="flex flex-col-reverse md:flex-row justify-center items-center md:items-stretch w-full gap-2 sm:gap-6 md:gap-12">
            ...
            <div class="flex flex-col gap-2 sm:gap-4 max-w-80 self-center sm:self-end text-center sm:text-left">
              <div class="overflow-hidden">
                <h3 :ref="(el) => { if (el) titleRefs[index] = el; }" class="uppercase font-bold text-2xl md:text-4xl">
                  <span v-if="item.title1" v-html="$t(item.title1)"></span><br />
                  <span v-if="item.title2" v-html="$t(item.title2)" class="text-accent"></span>
                </h3>
              </div>
              <div class="overflow-hidden">
                <p :ref="(el) => { if (el) descriptionRefs[index] = el; }" v-html="$t(item.desc)" class="text-sm md:text-base uppercase"></p>
              </div>
            </div>
          ...
          <div class="w-full md:w-1/2 flex justify-center md:justify-start">
            <div :class="[imageClass, 'overflow-hidden rounded-xl relative shrink-0']">
              <img :ref="(el) => { if (el) imageRefs[index] = el; }" :src="item.img" :alt="$t(item.title1)" class="w-full h-full object-cover origin-center absolute inset-0" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="flex flex-col items-center md:items-start gap-2 sm:gap-4 md:absolute md:bottom-10 md:left-8 z-30">
      ...
      <div class="w-full text-center text-xl md:text-2xl font-light tabular-nums">
        ...
        <span class="text-black/50">{{ slides?.length || 0 }}</span>
      </div>
