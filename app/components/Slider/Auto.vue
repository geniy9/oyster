...
<template>
    ...
    >
        <div class="relative grid grid-cols-1 w-full grow">
            ...
            >
                <div
                    class="flex flex-col-reverse md:flex-row justify-center items-center md:items-stretch w-full gap-2 sm:gap-6 md:gap-12"
                >
                    <!-- TEXT -->
                    <div
                        class="w-full md:w-1/2 flex flex-col justify-center items-start relative z-10 px-4 md:px-0 md:pl-8 pb-2 md:pb-0"
                    >
                        <div
                            class="flex flex-col gap-2 sm:gap-4 max-w-80 self-center sm:self-end text-center sm:text-left"
                        >
                            <div class="overflow-hidden">
                                <h3
                                    :ref="
                                        (el) => {
                                            if (el) titleRefs[index] = el;
                                        }
                                    "
                                    class="uppercase font-bold text-2xl md:text-4xl"
                                >
                                    <span v-html="$t(item.title1)"></span><br />
                                    <span
                                        v-html="$t(item.title2)"
                                        class="text-accent"
                                    ></span>
                                </h3>
                            </div>
                    ...
                    >
                        <div
                            class="w-72 h-128 overflow-hidden rounded-xl relative shrink-0"
                        >
                            <img
                                :ref="
                                    (el) => {
        ...
        <!-- NAV (COUNTER + BUTTONS) -->
        <div
            class="flex flex-col items-center md:items-start gap-2 sm:gap-4 md:absolute md:bottom-10 md:left-8 z-30"
        >
            <!-- COUNTER -->
            <div
                class="w-full text-center text-xl md:text-2xl font-light tabular-nums"
            >
            ...
            <div class="flex items-center gap-2">
                ...
                <div
                    @click="onClickPrev"
                    class="group flex items-center justify-center p-2 w-10 h-10 rounded-full border border-black cursor-pointer hover:bg-black hover:text-white transition-colors duration-300"
                >
                    <UIcon
