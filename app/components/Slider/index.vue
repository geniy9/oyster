...
const items = [...Array(props.qty).keys()].map(i => `/img/service/1.jpg`);
...
<template>
  <div class="flex flex-col items-start gap-2">
    <UCarousel ref="carousel" :items="items" v-slot="{ item, index }" loop 
    class="w-xl" :ui="{ item:'flex flex-row items-center gap-8' }">
    
      
      <div class="mb-4 overflow-hidden rounded-xl">
        <img :ref="el => { if(el) imageRefs[index] = el }"
          :src="item" 
          :alt="$t(`title.service_${index + 1}.name`)" 
          class="w-64 h-96 object-cover origin-center" />
      </div>
    </UCarousel>

    <div class="flex flex-col h-32 w-80">
        <div class="overflow-hidden">
          <h3 :ref="el => { if(el) titleRefs[currentIndex] = el }" class="uppercase font-bold text-lg leading-5">
            {{ $t(`title.service_${currentIndex + 1}.name`) }}
          </h3>
        </div>
        <div class="overflow-hidden">
          <p :ref="el => { if(el) descriptionRefs[index] = el }" class="text-sm">
            {{ $t(`title.service_${currentIndex + 1}.desc`) }}
          </p>
        </div>
      </div>
