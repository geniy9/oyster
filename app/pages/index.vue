...
const zoomingImages = [
  { desktop: "/img/blue_face.jpg", tablet: "/img/blue_face_tab.jpg", mobile: "/img/blue_face_mob.jpg" },
  { desktop: "/img/hologram_green.jpg", tablet: "/img/hologram_green_tab.jpg", mobile: "/img/hologram_green_mob.jpg" },
  { desktop: "/img/render_3d.jpg", tablet: "/img/render_3d_tab.jpg", mobile: "/img/render_3d_mob.jpg" },
  { desktop: "/img/sea_water.jpg", tablet: "/img/sea_water_tab.jpg", mobile: "/img/sea_water_mob.jpg" },
];
...
const carouselServices = computed(() => {
  const items = [];
  for (let i = 1; i <= 4; i++) {
    items.push({
      index: i,
...
const carouselCases = computed(() => {
  ...
  for (let i = 1; i <= 8; i++) {
    items.push({
      index: i,
      img: `/img/case/${i}.jpg`,
      title1: `title.case_${i}.name`, 
      title2: '', 
      desc: `title.case_${i}.desc`,
    });
  }
  return items;
});
...
const marqueeItems = ["promotion", "design", "target", "leads", "content", "consulting", "automation", "research", "development"];

const advantageSection = ref(null);
const rotatingImage = ref(null);
const advantageSlides = ref([]);
const advantageImages = ["/img/advantage/1.jpg", "/img/advantage/2.jpg", "/img/advantage/3.jpg", "/img/advantage/4.jpg"];
...
function initGsap() {
  ...
  mm.add({
    ...
    splitTitle.value = new SplitText(titles, { type: "words,lines", linesClass: "line", autoSplit: true, mask: "lines" });
    gsap.from(splitTitle.value.lines, { yPercent: 120, delay: 3.6, duration: 1, opacity: 0, ease: "expo.out", stagger: 0.15, zIndex: 0 });

    // VERTICAL LINES
    const curtainTL = gsap.timeline({
      scrollTrigger: {
        trigger: "#line_container", start: "top top", end: "+=200%", scrub: 1, pin: true, pinType: isMobile ? "fixed" : "transform",
      },
    });

    curtainTL.to([leftLineGrow.value, rightLineGrow.value], { height: "100vh", duration: 1, ease: "none", zIndex: 10 });
    curtainTL.call(() => { footerNav.value = true; }, [], ">");
    curtainTL.call(() => { footerNav.value = false; }, [], ">");
    curtainTL.to([leftLineGrow.value, rightLineGrow.value], { width: isMobile ? "100vw" : "50vw", duration: 1, ease: "none" });

    // ZOOMING
    const scalingTL = gsap.timeline({
      scrollTrigger: {
        trigger: "#zooming_section", start: "top top", end: "+=300%", scrub: 1, pin: true, pinType: isMobile ? "fixed" : "transform",
      },
    });
    ...
    zoomingImgs.value.forEach((img, index) => {
      scalingTL.to(img, {
        scale: 1, duration: 1, ease: "none", delay: 0.5,
    ...
    scalingTL.fromTo(aboutNitro.value, { opacity: 1, duration: 0.1, ease: "none" }, { yPercent: 0, duration: 2, ease: "none" }, "<");
    ...
    if (advantageSection.value) {
      ...
      for (let i = 0; i < total - 1; i++) {
        const nextIndex = i + 1;
        if (isMobile) {
          advantageTL.to(slides[i], { autoAlpha: 0, yPercent: -20, ease: "none" })
        ...
        advantageTL.to(rotatingImage.value, { rotationY: "+=180", ease: "none" }, label);
        const isBackSide = nextIndex % 2 !== 0;
        advantageTL.set(rotatingImage.value, { 
          attr: { src: advantageImages[nextIndex] }, 
          scaleX: isBackSide ? -1 : 1, immediateRender: true 
        }, "<50%");
      }
    }
  ...
  <div class="w-full relative">
    <section id="hero" class="relative flex flex-col justify-center items-center h-screen">
      <div class="lg:min-w-lg xl:min-w-2xl" aria-hidden="true">
        ...
        <div class="text-xs xs:text-base sm:text-lg font-bold uppercase relative z-10">
          <p>
            {{ $t("title.for_your_business") }}
          </p>
        </div>
      </div>
      <div id="line_container" class="absolute w-screen h-screen overflow-hidden">
        <div ref="leftLineGrow" class="fixed top-1/2 left-1/2 h-0 w-2 -mr-0.5 -translate-y-1/2 -translate-x-full bg-primary"></div>
        <div ref="rightLineGrow" class="fixed top-1/2 left-1/2 h-0 w-2 -ml-0.5 -translate-y-1/2 bg-primary"></div>
      </div>
    </section>

    <section id="zooming_section">
      <div class="relative w-screen h-screen overflow-hidden">
        <picture v-for="(item, index) in zoomingImages" :key="index" :ref="(el) => { if (el) zoomingImgs[index] = el; }" class="zooming_img" :style="{ zIndex: index + 1 }">
        ...
        <div id="about" ref="aboutNitro" class="absolute top-0 left-0 w-full h-full bg-white py-8">
          <div class="section">
            <div class="px-4 sm:px-20 md:px-32 mb-8">
              ...
              <p v-html="$t('title.about.desc')" class="text-lg sm:text-xl md:text-2xl ml-0 md:ml-20 uppercase"></p>
            </div>
    ...
    <section id="numbers" class="relative">
      <div class="sticky top-0 h-screen w-full overflow-hidden">
        <div v-for="(item, i) in aboutItems" :key="i" :ref="(el) => { if (el) aboutFrames[i] = el; }" class="absolute top-0 left-0 w-full h-full bg-cover bg-center bg-no-repeat" :style="{ backgroundImage: `url(/img/bg_about_${i + 1}.jpg)` }">
          <div :ref="(el) => { if (el) aboutTextContainers[i] = el; }" class="relative w-full h-full p-4">
            ...
            <div class="relative flex flex-col items-stretch justify-evenly h-full max-w-4xl mx-auto text-center z-5">
              ...
              <p data-split-text v-html="$t(`title.about_${i + 1}.desc`)" class="text-xl font-light uppercase text-white"></p>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="cases" class="relative bg-cover bg-center bg-no-repeat sm:bg-fixed bg-[url(/img/bg_cases_mob.jpg)] xs:bg-[url(/img/bg_cases_tab.jpg)] lg:bg-[url(/img/bg_cases.jpg)]">
      <div class="min-h-screen w-full flex items-center justify-center py-10">
        ...
        <div class="block sm:hidden w-full">
          <SliderAuto :slides="carouselCases" image-class="w-80 h-60" class="section" />
        </div>
        <!-- DESKTOP -->
        <div class="section hidden sm:grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-6 w-full h-full">
    ...
    <section id="services" ref="serviceSection" class="relative min-h-screen bg-white">
      ...
      <div class="flex items-center justify-center">
        <SliderAuto :slides="carouselServices" class="section" />
      </div>
    </section>
    ...
    <section ref="advantageSection" class="relative overflow-hidden bg-gray-200 h-screen">
      <div class="relative z-5 flex flex-col justify-evenly sm:grid sm:grid-cols-2 min-h-screen">
        <div class="relative overflow-hidden h-[45vh] sm:h-full flex items-center justify-center sm:block">
          <div class="relative sm:block w-full">
            <div v-for="(n, i) in 4" :key="i" :ref="(el) => (advantageSlides[i] = el)" class="flex items-center justify-center sm:justify-end px-6 h-full sm:h-screen">
        ...
        <div class="relative h-[55vh] sm:h-screen flex items-center justify-center sm:justify-start sm:border-l sm:border-gray-300 sm:pl-[10%] bg-cover bg-center bg-no-repeat sm:bg-fixed sm:bg-[url(/img/bg_advantage.jpg)]">
          <div style="perspective: 1000px" class="transform-3d">
            <img ref="rotatingImage" :src="advantageImages[0]" class="w-60 xl:w-80 transform-3d rounded-lg" />
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="section">
        <div class="grid grid-cols-1 md:grid-cols-3 items-end gap-8 text-sm">
          ...
          <div class="flex flex-col items-center md:items-end gap-6 text-center md:text-right">
            <a href="https://t.me/oystercomputer" target="_blank" class="flex items-center justify-center p-2 w-44 rounded-full border border-white hover:bg-black hover:text-white transition-colors duration-300 uppercase text-xs">
              {{ $t("text.write_to_us") }}
            </a>
